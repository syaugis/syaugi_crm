@push('scripts')
    ...
    <script>
        const myModal = document.getElementById('ImportModalLead')
...
<x-app-admin-layout :assets="['datatables']">
    @php
        $headerAction = [
            ...
            '" class="btn btn-sm btn-primary" role="button">Tambahkan Calon Pelanggan</a>',
            '<button type="button" class="btn btn-sm btn-warning" data-bs-toggle="modal" data-bs-target="#ImportModalLead"> Impor Calon Pelanggan </button>',
            '<a href="' .
            route('admin.lead.export') .
            '" class="btn btn-sm btn-success" role="button">Ekspor Calon Pelanggan</a>',
        ];
    @endphp
    <x-data-table :pageTitle="'Data Calon Pelanggan'" :headerAction="$headerAction ?? ''" :dataTable="$dataTable" />
    <x-modal modalId="ImportModalLead" title="Impor Calon Pelanggan" action="{{ route('admin.lead.import') }}"
        method="POST" submitLabel="Impor">
        <x-file name="file" id="file" label="Excel File" placeholder="Masukkan File Excel"
            accept="application/vnd.ms-excel, application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
            required="true" />

        <a href="{{ route('admin.lead.template') }}"
            class="btn btn-primary d-flex align-items-center justify-content-center" data-bs-toggle="tooltip"
            data-bs-placement="top" title="Unduh Template Excel untuk Impor Calon Pelanggan">
