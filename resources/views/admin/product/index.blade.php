...
<x-app-admin-layout :assets="['datatables']">
    @php
        $headerAction = [
            ...
            '" class="btn btn-sm btn-primary" role="button">Tambahkan Produk</a>',
            '<button type="button" class="btn btn-sm btn-warning" data-bs-toggle="modal" data-bs-target="#ImportModalProduct"> Impor Produk </button>',
            '<a href="' .
            route('admin.product.export') .
            '" class="btn btn-sm btn-success" role="button">Ekspor Produk</a>',
        ];
    @endphp
    <x-data-table :pageTitle="'Data Produk'" :headerAction="$headerAction ?? ''" :dataTable="$dataTable" />
    <x-modal modalId="ImportModalProduct" title="Impor Produk" action="{{ route('admin.product.import') }}" method="POST"
        submitLabel="Impor">
        ...
        <a href="{{ route('admin.product.template') }}"
            ...
            data-bs-placement="top" title="Unduh Template Excel untuk Impor Produk">
